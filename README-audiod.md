...
* **elogind** (any reasonably recent version that exposes the login1 D-Bus
  interface with `UserNew`/`UserRemoved` — verified on 257.16).
...
```sh
# from the SlackBuild kit (contains the SlackBuild + src/ tree):
tar xzf audiod-slackbuild.tar.gz
cd audiod
sudo bash audiod.SlackBuild          # produces /tmp/audiod-<ver>-noarch-<b>_rtz.txz
